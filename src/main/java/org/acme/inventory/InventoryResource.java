...
import org.acme.users.UsersResource;
...
public class InventoryResource  {
    ...
    @GET
    public Uni<List<Inventory>> get() {
        return Inventory.listAll(Sort.by("name"));
    }

    @GET
    @Path("{id}")
    @ReactiveTransactional
    public Uni<Inventory> getSingle(@RestPath Long name) {
...
//
    @POST
    public void create(Inventory fruit) {
        if (fruit == null || fruit.id != null) {
            throw new WebApplicationException("Id was invalidly set on request.", 422);
        }
        inventoryRepository.persist(fruit);
